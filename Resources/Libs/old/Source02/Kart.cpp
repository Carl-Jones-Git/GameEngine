...
Kart::Kart(FMOD::System* _FMSystem,ID3D11Device* device, ID3D11DeviceContext* context, Terrain *_ground, shared_ptr<Effect>  _perPixelEffect,  shared_ptr<Texture> kartTexture)
{
	...
	FMSystem = _FMSystem;
	FMOD_RESULT result;
	result = FMSystem->createSound("../Resources/Sounds/Two Stroke Revving2.mp3", FMOD_DEFAULT | FMOD_LOOP_NORMAL, nullptr, &engine2);
	result = FMSystem->createSound("../Resources/Sounds/Two Stroke Revving3.mp3", FMOD_DEFAULT | FMOD_LOOP_NORMAL, nullptr, &engine3);
	if (result != FMOD_OK)
		// Handle error
	...
	// Load Tire Texture
	Texture tireTexture = Texture(context, device, L"..\\Resources\\Models\\Kart\\GoodYearEagle.png");
	...
	Frame = new  Model(context,device, wstring(L"..\\Resources\\Models\\Kart\\KartLowPoly.obj"), perPixelEffectCullNone, glossBlack, 0,&FramePreTransMat);
	//Load Faring
	...
	Farings = new  Model(context, device, wstring(L"..\\Resources\\Models\\Kart\\FaringTexture2.obj"), _perPixelEffect, matFaring, DEFAULT,&FaringPreTransMat);
	...
	//Load Engine
	Engine = new  Model(context, device, wstring(L"..\\Resources\\Models\\Kart\\KartLowPoly.obj"), _perPixelEffect, matChrome,  4,&FramePreTransMat);
	...
	FLWheel = new  Model(context, device, wstring(L"..\\Resources\\Models\\Kart\\KartLowPolyWC.obj"), _perPixelEffect, glossBlackReflection, 8, &Wheel2kartPreTransMat);
	FRWheel = new  Model(context, device, wstring(L"..\\Resources\\Models\\Kart\\KartLowPolyWC.obj"), _perPixelEffect, glossBlackReflection, 10, &Wheel2kartPreTransMat);
	FLTire = new  Model(context, device, wstring(L"..\\Resources\\Models\\Kart\\KartLowPolyWC.obj"), _perPixelEffect, matTire, 9, &Wheel2kartPreTransMat);
	FRTire = new  Model(context, device, wstring(L"..\\Resources\\Models\\Kart\\KartLowPolyWC.obj"), _perPixelEffect, matTire, 7, &Wheel2kartPreTransMat);
	RearWheels = new  Model(context, device, wstring(L"..\\Resources\\Models\\Kart\\KartLowPolyWC.obj"), _perPixelEffect, glossBlack, 6, &FramePreTransMat);
	RearTires = new  Model(context, device, wstring(L"..\\Resources\\Models\\Kart\\KartLowPolyWC.obj"), _perPixelEffect, matTire, 5, &Wheel2kartPreTransMat);
	...
	SWheel = new  Model(context, device, wstring(L"..\\Resources\\Models\\Kart\\KartLowPolyWC.obj"), _perPixelEffect, mattBlack, 2,&SWheelPreTransMat);
	SWheelHub = new  Model(context, device, wstring(L"..\\Resources\\Models\\Kart\\KartLowPolyWC.obj"), _perPixelEffect, mattBlack, 3,&SWheelPreTransMat);
	...
	Driver = new  Model(context, device, wstring(L"..\\Resources\\Models\\Driver\\GTR_Driver3Smooth.obj"), _perPixelEffect, mattGrey, 5,&DriverPreTransMat);
	Head = new  Model(context, device, wstring(L"..\\Resources\\Models\\Driver\\GTR_Driver3Smooth.obj"), _perPixelEffect, glossBlackReflection,  7, &DriverPreTransMat);
	Shoes = new  Model(context, device, wstring(L"..\\Resources\\Models\\Driver\\GTR_Driver3SmoothFeet.obj"), _perPixelEffect, mattBlack, DEFAULT, &DriverPreTransMat);
	Hands = new  Model(context, device, wstring(L"..\\Resources\\Models\\Driver\\GTR_Driver3SmoothHands.obj"), _perPixelEffect, mattGrey, DEFAULT, &HandsPreTransMat);
	Legs = new  Model(context, device, wstring(L"..\\Resources\\Models\\Driver\\GTR_Driver3SmoothLegs.obj"), _perPixelEffect, mattGrey,  DEFAULT, &DriverPreTransMat);
...
void Kart::updateTires(physx::PxTransform wheelTrans[],float stepSize)
{
	...
	for (PxU32 i = 0; i < 4; i++)
	{
		if (ground->getColourMap(trans.p.x + wheelTrans[i].p.x, trans.p.z + wheelTrans[i].p.z).x > 0.0)			//onGrass = true;
		{
