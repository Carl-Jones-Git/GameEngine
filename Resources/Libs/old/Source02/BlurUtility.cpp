...
void BlurUtility::initCBuffer(ID3D11Device *device, int _blurWidth, int _blurHeight) {
	...
	cBufferTextSizeCPU->GaussWidth = 1.0;
...
//void BlurUtility::updateScreenSize(System* system)
//{
//	D3D11_TEXTURE2D_DESC depthDesc;
//	ID3D11Resource* DSTex;
//	ID3D11ShaderResourceView* depthSRV = system->getDepthStencilSRV();
//	depthSRV->GetResource(&DSTex);
//	static_cast<ID3D11Texture2D*>(DSTex)->GetDesc(&depthDesc);
//	//cBufferTextSizeCPU->Width = depthDesc.Width;
//	//cBufferTextSizeCPU->Height = depthDesc.Height;
//	//cout << "width" << cBufferTextSizeCPU->Width << "Height" << cBufferTextSizeCPU->Height << endl;
//	updateTextSize(context, depthDesc.Width, depthDesc.Height);
//}

void BlurUtility::updateTextSize(ID3D11DeviceContext *context,int width,int height) {
...
void BlurUtility::blurModel(System* system,Model*obj)
{
	// Blur the object 
	if (obj) {
		...
		for (int i = 0; i < obj->instances.size(); i++)
		{
			...
			obj->update(context,i);
			obj->render(context,i);// Render using objects Effect
			obj->setWorldMatrix(XMMatrixScaling(1.1, 1.1, 1.1) * objMat, i);
			obj->update(context, i);
			obj->render(context, i);// Render using objects Effect
			obj->setWorldMatrix(XMMatrixScaling(1.2, 1.2, 1.2) * objMat, i);
			obj->update(context, i);
			obj->render(context, i);// Render using objects Effect
			obj->setWorldMatrix(objMat,i);
			obj->update(context,i);
		}
		...
		updateTextSize(context, offScreenViewport.Width, offScreenViewport.Height);
		context->PSSetConstantBuffers(0, 1, &cBufferTextSizeGPU);
		//Horizontal blur
		...
		//context->ClearRenderTargetView(tempRT[0], clearColorBlur);
...
void BlurUtility::blurModels(System* system, function<void(ID3D11DeviceContext*)> renderGlowObjects)
{
	// Ensure default states
	FLOAT			blendFactor[] = { 1, 1, 1, 1 };
	context->OMSetDepthStencilState(defaultEffect->getDepthStencilState(), 0);
	...
	context->OMSetRenderTargets(1, &intermedHRTV, nullptr);
	context->PSSetShaderResources(0, 1, &intermedSRV);
	//context->PSSetShaderResources(1, 1, &depthBlurSRV);
	context->VSSetShader(screenQuadVS, 0, 0);
	context->PSSetShader(horizontalBlurPS, 0, 0);
	screenQuad->render(context);
...
HRESULT BlurUtility::setupBlurRenderTargets(ID3D11Device *deviceIn, int blurWidth, int blurHeight){
	...
	updateTextSize(context, blurWidth, blurHeight);
	...
	if (SUCCEEDED(hr))
		hr = device->CreateShaderResourceView(tex, &shaderResourceViewDesc, &depthBlurSRV);
