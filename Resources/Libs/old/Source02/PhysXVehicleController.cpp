...
PxVehicleDrive4W* PhysXVehicleController::initVehiclePX(float x, float y, float z, float r,int id)
{
   ...
   // physx::PxFilterData chassisSimFilterData(COLLISION_FLAG_CHASSIS, COLLISION_FLAG_WHEEL, 0, 0);
    ...
    //physx::PxFilterData wheelSimFilterData(COLLISION_FLAG_WHEEL, COLLISION_FLAG_WHEEL, physx::PxPairFlag::eMODIFY_CONTACTS, 0);
...
{
    {
        ...
        //0.3f,	//rise rate eANALOG_INPUT_STEER_LEFT
        //0.3f,	//rise rate eANALOG_INPUT_STEER_RIGHT
...
VehicleDesc PhysXVehicleController::initVehicleDesc(PxMaterial* material,const PxFilterData& chassisSimFilterData, const PxFilterData& wheelSimFilterData, const PxU32 vehicleId)
{
   ...
   const PxF32 wheelMOI = 0.5f * wheelMass * wheelRadius * wheelRadius;
    //const PxF32 wheelMOI = 0.5f * 1.5 * wheelRadius * wheelRadius;
    ...
    //vehicleDesc.wheelSimFilterData = PxFilterData(COLLISION_FLAG_WHEEL, COLLISION_FLAG_WHEEL_AGAINST, 0, 0);// (COLLISION_FLAG_WHEEL, COLLISION_FLAG_WHEEL_AGAINST, 0, 0);
...
void PhysXVehicleController::computeWheelCenterActorOffsets4W(const PxF32 wheelFrontZ, const PxF32 wheelRearZ, const PxVec3& chassisDims, const PxF32 wheelWidth, const PxF32 wheelRadius, const PxU32 numWheels, PxVec3* wheelCentreOffsets)
{
 ...
 /*   wheelCentreOffsets[PxVehicleDrive4WWheelOrder::eREAR_LEFT] = PxVec3((-chassisDims.x - wheelWidth) * 0.5f, -(chassisDims.y / 2)-0.1, wheelRearZ + 0 * deltaZ * 0.5f);
    wheelCentreOffsets[PxVehicleDrive4WWheelOrder::eREAR_RIGHT] = PxVec3((+chassisDims.x + wheelWidth) * 0.5f, -(chassisDims.y /2 ) - 0.1, wheelRearZ + 0 * deltaZ * 0.5f);
    wheelCentreOffsets[PxVehicleDrive4WWheelOrder::eFRONT_LEFT] = PxVec3((-chassisDims.x - wheelWidth) * 0.5f, -(chassisDims.y /2) - 0.1, wheelRearZ + (numLeftWheels - 1) * deltaZ);
    wheelCentreOffsets[PxVehicleDrive4WWheelOrder::eFRONT_RIGHT] = PxVec3((+chassisDims.x + wheelWidth) * 0.5f, -(chassisDims.y / 2 )- 0.1, wheelRearZ + (numLeftWheels - 1) * deltaZ);*/
