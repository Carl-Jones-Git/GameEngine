...
{ 
	...
	SIZE_T shaderBytes = LoadShader("Shaders\\cso\\basic_texture_vs.cso", &tmpShaderBytecode);
	system->getDevice()->CreateVertexShader(tmpShaderBytecode, shaderBytes, NULL, &screenQuadVS);
	system->getDevice()->CreateInputLayout(basicVertexDesc, ARRAYSIZE(basicVertexDesc), tmpShaderBytecode, shaderBytes, &screenQuadVSInputLayout);
	free(tmpShaderBytecode);

	shaderBytes = LoadShader("Shaders\\cso\\basic_texture_ps.cso", &tmpShaderBytecode);
	system->getDevice()->CreatePixelShader(tmpShaderBytecode, shaderBytes, NULL, &screenQuadPS);
	free(tmpShaderBytecode);


	backGroundTex = new Texture(system->getDevice(), L"..\\Resources\\Textures\\Intro02.png");
	...
	backGround = new Quad(system->getDevice(), screenQuadVSInputLayout);
};
...
void Menu::initMainMenu()
{
	...
	backGroundTex = new Texture(system->getDevice(), L"..\\Resources\\Textures\\Garage04.png");
	//shared_ptr<Effect>tmp(new Effect(system->getDevice(), "Shaders\\cso\\per_pixel_lighting_vs.cso", "Shaders\\cso\\planarShadow_ps.cso", extVertexDesc, ARRAYSIZE(extVertexDesc)));
	planarShadowEffect = make_shared<Effect>( system->getDevice(), "Shaders\\cso\\per_pixel_lighting_vs.cso", "Shaders\\cso\\planarShadow_ps.cso", extVertexDesc, ARRAYSIZE(extVertexDesc));
	...
	rState->Release();
	system->getDevice()->CreateRasterizerState(&rSDesc, &rState);
	...
	system->getDevice()->CreateDepthStencilState(&updateSStateDesc, &updateSState);
...
void Menu::renderMainMenu(PlayerKart *playerKart, MenuState* MainMenuState, float stepSize)
{
	...
	ImGui::NewFrame();
	{
		...
		if (*MainMenuState == MainMenu)
		{
			...
			if (ImGui::Button("Play Offline"))//|| (ImGui::IsKeyPressed(ImGuiKey_Enter)&& focusedButtonIndex==2))
			{
				//acc = 0.0f;
				*MainMenuState = MenuDone;
				//mainClock->tick();
