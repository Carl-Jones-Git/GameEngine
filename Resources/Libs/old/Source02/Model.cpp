...
void Model::load(ID3D11Device *device,  const std::wstring& filename) {
	...
	try
	{
		...
		HRESULT hr = 0;// loadModelAssimp(device, filename);
...
//HRESULT Model::loadModelAssimp(ID3D11DeviceContext* context, ID3D11Device* device, const std::wstring& filename)
//{
//	HRESULT hr=loadModelAssimp(device, filename);
//	for (int i = 0;i < numTextures; i++)
//		if (textures[i])
//		{
//			context->GenerateMips(textures[i]);
	...
	try
	{
		...
		if (meshNumber == NO_JOIN)
			...
			scene = importer.ReadFile(filename_string, aiProcess_PreTransformVertices | aiProcess_GenSmoothNormals | aiProcess_CalcTangentSpace |
				...
				aiProcess_SortByPType);
		//else if (meshNumber == -4)
		//	scene = importer.ReadFile(filename_string, aiProcess_PreTransformVertices | aiProcess_GenNormals | aiProcess_CalcTangentSpace |
		//		aiProcess_Triangulate |
		//		aiProcess_JoinIdenticalVertices |
		//		aiProcess_SortByPType);
		...
		for (uint32_t k = 0; k < numMeshes; ++k)
		{
			...
			if (meshNumber == k || meshNumber<=-1)
			{
				...
				{
					//indexCount.push_back(mesh->mFaces[i].mNumIndices);
		...
		vertexBufferCPU = (ExtendedVertexStruct*)malloc(numVertices * sizeof(ExtendedVertexStruct));
			...
			for (uint32_t i = 0; i < numMeshes; ++i) 
			{
				...
				if (instances[0].materials.size() > i)
				{
					diffuse = XMCOLOR(instances[0].materials[i]->getColour()->diffuse.x, instances[0].materials[i]->getColour()->diffuse.y, instances[0].materials[i]->getColour()->diffuse.z, instances[0].materials[i]->getColour()->diffuse.w);
					specular = XMCOLOR(instances[0].materials[i]->getColour()->specular.x, instances[0].materials[i]->getColour()->specular.y, instances[0].materials[i]->getColour()->specular.z, instances[0].materials[i]->getColour()->specular.w);
				}
