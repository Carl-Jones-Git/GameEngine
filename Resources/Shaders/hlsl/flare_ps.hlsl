/*
 * Educational Game Engine - Lense Flare Shader
...
FragmentOutputPacket main(FragmentInputPacket p) {
	...
	float g;
	if (p.colour.r + p.colour.g + p.colour.b > 2.9f)
		g = flareTexture.Sample(linearSampler, p.texCoord).r*0.7;
	else
		g = flareTexture.Sample(linearSampler, p.texCoord).r * 0.2;
