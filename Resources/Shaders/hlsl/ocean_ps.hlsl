//
// Basic Water Simulation - Modified from NVidia Ocean.fx by Carl Jones 2023
//
// NVIDIA Comments :
/****************************************************************************
...
FragmentOutputPacket main(FragmentInputPacket IN) { 
	...
	float3 DeepColor = { 0.0f, 0.0f, 0.0f };
	float3 ShallowColor = { 0.1f, 0.05f, 0.0f };
	float3 ReflTint =		{ 1.0f, 0.0f, 1.0f };
	float Kr =				1.0f;
	float KWater =			1.0f;
	float HDRMultiplier =	0;

	// Sum normal maps
	float4 t0 = gNormMap.Sample(gLinearSam, IN.bumpUV0)*2.0 - 1.0;
	float4 t1 = gNormMap.Sample(gLinearSam, IN.bumpUV1)*2.0 - 1.0;
	float4 t2 = gNormMap.Sample(gLinearSam, IN.bumpUV2)*2.0 - 1.0;
	...
	// Reflection
	float3 Vn = normalize(IN.eyeDir);// normalize(IN.eyeDir + float3(0.0, 1.0, 0.0));
	float3 R = reflect(-Vn, Nn);
	float3 reflection = gCubeMap.Sample(gLinearSam, R).rgb;
	//reflection = reflection * 0.5 + reflection * reflection * 0.5;
	//reflection.rgb *= reflection.rgb;// (reflection.r + reflection.g + reflection.b / 3);
	//reflection.rgb *= (1.0 + reflection.a*HDRMultiplier); //notused
	
	// Fresnel
	float facing = 1.0 - max(dot(Vn, Nn), 0);
	float3 waterColor = DeepColor;// KWater* lerp(DeepColor, ShallowColor, facing);
	// Modify Code Here (Complete Fresnel calculation)
	float fres = Kr*(FresnelBias + (1.0 - FresnelBias)*pow(facing, FresnelExp));

	float3 colour = waterColor + ( reflection.rgb*fres);// *ReflTint);
	//add fog
	float d = length(IN.posW.xyz - eyePos.xyz);
	...
	outputFragment.fragmentColour = float4(colour.rgb, fres);
