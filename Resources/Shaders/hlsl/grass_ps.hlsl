...
float inShadow(FragmentInputPacket v, float zBias = 0.0001f)
{
	...
	if (v.posSdw.x < 1 && v.posSdw.x>0 && v.posSdw.y < 1 && v.posSdw.y>0)
	{
		...
		if (dist < 5.0f)
		{
			...
			//percentLit = min(shadowMap.SampleCmp(SamShadow, v.posSdw.xy, v.posSdw.z + zBias), percentLit);
		}
		else
...
FragmentOutputPacket main(FragmentInputPacket v) {
	//clip((v.posW.y < 0.8f && length(v.posW.xz - float2(-54.0, 66.0)) < 15) ? -1 : 1);
	...
	//outputFragment.fragmentColour = float4(noise2Map.Sample(anisotropicSampler, v.texCoord).xyz , 1);
	...
	if ((baseColour.g < 0.06 || baseColour.b > 0.6 )|| (baseColour2.g < 0.06 || baseColour2.b > 0.6) || (baseColour3.g < 0.06 || baseColour3.b > 0.6))
	{
		...
		else//Track Colour
		{
			...
			//baseColour += asphaltTexture.Sample(anisotropicSampler, v.texCoord * 40) / 3 + 0.2;
			//Darken Colour Slightly
			baseColour *= 0.7;
		}
		...
		if (length(v.posW.xyz - float3(-54.0, 1.0, 66.0)) < 15)
		{
			...
			if (v.posW.y > waterLineTop && v.posW.y < fadeTop)
			{
				...
				else
				{
					...
					//normalMapWeight = lerp(normalMapWeight, 0, w);
				}		
			}

			//else if (v.posW.y < fadeBottom - fadeRange)
			else if (v.posW.y <= waterLineTop)
			{
				...
				specAmount = 0;
				specPower = 5;
				//float w = max((v.posW.y - (fadeBottom- fadeRange - fadeRange)) / (fadeRange*2),0);
				float w = max((v.posW.y - waterLineBottom) / (waterLineRange), 0);
				//if (w > 0.5)
					baseColour =  lerp(mudcolour * 0.8, mudcolour, saturate((w - 0.5) * 2));
				//specAmount = lerp( 0.0, 0.1, w);
					if (w <= 0.5)
					{
						specAmount = lerp(0.25, specAmount, saturate(w * 2));
						//specPower = lerp(5, specPower, saturate(w * 2));;

					}
					

				
				//specAmount = lerp(0.4, 0.0, w);
				//if (w > 0.5)
				//	specAmount = lerp(0.4, 0.0, (w - 0.5) * 2);
				//else
				//	specAmount = lerp(mudcolour*0.2, 0.4, w * 2.0);
	...
	////Lighting
	//colour = baseColour * lightAmbient;
	...
	//float3 lamb = max(dot(lightDir, N), 0.0f) * baseColour * lightDiffuse;
	//lamb *= min(percentLit + 0.3, 1.0);
	////add diffuse light
	//colour += lamb;
	...
	if (grassHeight > 0.0)//We are rendering a grass shell
	{
		...
		//colour.r = noiseMap.Sample(anisotropicSampler, v.texCoord * tileRepeat).r;
		...
		clip((alpha < 0.1f) ? -1 : 1);
		//colour = float3(alpha, alpha, alpha);
		//colour *= max((0.3 + (1.0f - alpha)) * ((distFadeStart + distFadeRange) - dist) / (distFadeStart + distFadeRange) * 2.0, 1.0f);
		//colour += float3(0.5, 1, 0) * grassHeight * 300 * max((0.3 + (1.0f - alpha)) * ((distFadeStart + distFadeRange) - dist) / (distFadeStart + distFadeRange) * 2.0, 1.0f);
		//if (dist > distFadeStart)
		{
			float3 tipColour = lights[0].Diffuse.rgb * float3(0.025, 0.05, 0);
			//float3 colourNear = lerp(colour - float3(0.05, 0.1, 0), colour + float3(0.05, 0.1, 0), relativeGrassHeight);
			//colour=lerp(colour, lerp(colour - float3(0.05, 0.1, 0), colour + float3(0.05, 0.1, 0), relativeGrassHeight),fadeFactor);
		...
		//alpha = min(fadeFactor, alpha);
		//colour *= relativeGrassHeight*2-0.5f ;
		//colour.xyz = max(fadeFactor,0);
		
		}
		//outputFragment.fragmentColour = float4(colour, alpha);
		//return outputFragment;
		//
		//colour *= 0.2 + (1.0f - alpha) * (1 + grassHeight * 10);// alpha + (grassHeight);// *(1 - alpha) * 3;
		//colour = float3(1, 1, 1);
		//alpha = 1.0; 
		//colour += float3(0.5, 1, 0)* grassHeight*300;
		//alpha = grassAlpha.Sample(anisotropicSampler, v.texCoord * tileRepeat).a;
	}
	//else
	//	colour += float3(0.5, 1, 0) * (1.0f - (dist - distFadeStart) / distFadeRange)*0.001;
