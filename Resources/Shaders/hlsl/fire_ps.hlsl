...
//Texture2DMS depth: register(t5); //could be used for soft particles
...
FragmentOutputPacket main(FragmentInputPacket p) {
	...
	float alpha = p.alpha * (col.x + col.y + col.z)/3;
	outputFragment.fragmentColour = float4(col.xyz*lightDiffuse, alpha);
