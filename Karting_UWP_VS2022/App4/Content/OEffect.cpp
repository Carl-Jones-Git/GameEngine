...
Effect::Effect(ID3D11Device *device, const char *vertexShaderPath, const char * pixelShaderPath, const char * geometryShaderPath, const D3D11_INPUT_ELEMENT_DESC vertexDesc[], UINT numVertexElements,  const D3D11_SO_DECLARATION_ENTRY streamOutDesc[], UINT numSOElements)
{
	...
	if (strcmp(pixelShaderPath, "") == 0)
		...
		CreatePixelShader(device, pixelShaderPath, &tmpShaderBytecode, &PixelShader);
	CreateGeometryShaderWithSO(device, geometryShaderPath, &tmpShaderBytecode, &GeometryShader, streamOutDesc, numSOElements);
...
HRESULT Effect::CreateGeometryShaderWithSO(ID3D11Device *device, const char *filename, char **GSBytecode, ID3D11GeometryShader **_geometryShader,const D3D11_SO_DECLARATION_ENTRY streamOutDesc[], UINT numSOElements) {
	...
	HRESULT hr = device->CreateGeometryShaderWithStreamOutput(GSBytecodeLocal, shaderBytes, streamOutDesc,
		numSOElements, NULL, 0, 0, NULL, &GeometryShader);
