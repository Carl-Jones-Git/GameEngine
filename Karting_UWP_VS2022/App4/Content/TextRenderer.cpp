...
{
	// Update display text.
	uint32 fps = timer.GetElapsedSeconds();

	//m_text = L"Hello No Errors";// (fps > 0) ? std::to_wstring(fps) + L" FPS" : L" - FPS";
...
void TextRenderer::Update(std::wstring text)
{
	// Update display text.
	//uint32 fps = timer.GetElapsedSeconds();
...
void TextRenderer::Render()
{
	...
	D2D1::Matrix3x2F screenTranslation = D2D1::Matrix3x2F::Translation(
		logicalSize.Width - m_textMetrics.layoutWidth-40,
		logicalSize.Height - m_textMetrics.height-40
		);
