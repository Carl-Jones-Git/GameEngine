...
Effect::Effect(ID3D11Device *device, const char *vertexShaderPath, const char * pixelShaderPath, const char * geometryShaderPath, const D3D11_INPUT_ELEMENT_DESC vertexDesc[], UINT numVertexElements,  const D3D11_SO_DECLARATION_ENTRY streamOutDesc[])
{
	char *tmpShaderBytecode = nullptr;
	uint32_t tmpVSSizeBytes = CreateVertexShader(device, vertexShaderPath, &tmpShaderBytecode, &VertexShader);
	device->CreateInputLayout(vertexDesc, numVertexElements, tmpShaderBytecode, tmpVSSizeBytes, &VSInputLayout);
	if (strcmp(pixelShaderPath, "") == 0)
		...
		CreatePixelShader(device, pixelShaderPath, &tmpShaderBytecode, &PixelShader);
	CreateGeometryShaderWithSO(device, geometryShaderPath, &tmpShaderBytecode, &GeometryShader, streamOutDesc);
	initDefaultStates(device);
}
...
uint32_t Effect::CreateVertexShader(ID3D11Device *device, const char *filename, char **VSBytecode, ID3D11VertexShader **vertexShader){
	...
	cout << "Loading Vertex Shader: " << filename << endl;
...
HRESULT Effect::CreatePixelShader(ID3D11Device *device, const char *filename, char **PSBytecode, ID3D11PixelShader **pixelShader)
{
	...
	cout << "Loading Pixel Shader: " << filename << endl;
...
HRESULT Effect::CreateGeometryShaderWithSO(ID3D11Device *device, const char *filename, char **GSBytecode, ID3D11GeometryShader **_geometryShader,const D3D11_SO_DECLARATION_ENTRY streamOutDesc[]) {
	...
	UINT particleSOSizes[] = { sizeof(streamOutDesc) };

	//printf("dec size is%d", sizeof(pDecl));


	HRESULT hr = device->CreateGeometryShaderWithStreamOutput(GSBytecodeLocal, shaderBytes, streamOutDesc,
			//7, particleSOSizes, 0, D3D11_SO_NO_RASTERIZED_STREAM, NULL, &GeometryShader);
		7, NULL, 0, D3D11_SO_NO_RASTERIZED_STREAM, NULL, &GeometryShader);
...
HRESULT Effect::CreateGeometryShader(ID3D11Device *device, const char *filename, char **GSBytecode, ID3D11GeometryShader **geometryShader)
{
	...
	cout << "Loading Geometry Shader: " << filename << endl;
...
HRESULT Effect::CreateHullShader(ID3D11Device *device, const char *filename, char **HSBytecode, ID3D11HullShader **hullShader)
{
	...
	cout << "Loading Hull Shader: " << filename << endl;
	//Load the compiled shader byte code.
...
HRESULT Effect::CreateDomainShader(ID3D11Device *device, const char *filename, char **DSBytecode, ID3D11DomainShader **domainShader)
{
	...
	cout << "Loading Domain Shader: " << filename << endl;
	//Load the compiled shader byte code.
